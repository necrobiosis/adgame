...
/**
 * 方阵的目标纵深（排数 × 排距）。超过就**原地压密**，不再往后拉长。
 *
 * 这条是整个后期难度的症结所在。以前列数封顶 5、排数不封顶，于是三百人的
 * 方阵是一条 6 米宽、七十八米长的纵队：
 *  · 只有最前面三排（十五个人）够得着被咬，后面两百八十五个是永远碰不到的血库；
 *  · 所有 AoE（Boss 践踏、酸液、自爆）覆盖的都是固定的几排，杀的是**固定人数**，
 *    不是固定比例——队伍越大，AoE 越不痛不痒。
 * 结果就是兵力越堆越无敌，满配后期零伤亡。
 *
 * 封顶之后，人多只会让同一块地方站得更挤：AoE 圈里的人数跟着涨，接触面也
 * 跟着变宽（能咬到人的僵尸变多）。一发践踏该带走一片，就真的带走一片。
 */
export const FORMATION_MAX_ROWS = 20;

/** 压密的下限：再挤也不能挤到人和人重叠。 */
export const SLOT_MIN_SCALE = 0.5;
 ...
 * 整队才有可能一起挪出圈外。
 */
export const FORMATION_MAX_COLS = 7;
...
export const RANK_FIRE = {
   ...
   * 算火力衰减时用的参考列宽。
   *
   * 衰减看的是"我前面挡着多少人"，这跟队形摆成几列没关系。直接用 u.row
   * 的话，方阵一压密（列数变多、排数变少）所有人都往前排挤，全队 DPS 会
   * 凭空暴涨一大截。固定用这个参考宽度换算，队形怎么变，火力曲线不变。
   */
  refCols: 5,
  /** 前几排不衰减。 */
  freeRows: 3,
...
export const BLOCK = {
   ...
   */
  /**
   * 军械门：一直吊在方阵正前方，永远走不到，也永远不会被撞上。
   *
   * 别的东西都在往后流——路面、废车、尸潮、金币墙——只有它钉在你正前方
   * 那么远的地方不动。于是它不再是"一个一秒半的射击窗口"，而是一整关都
   * 摆在那儿的一道选择题：**这一波压力不大的时候，要不要拐到它那一排上
   * 去啃两口？**
   *
   * 啃它的代价很实在：你的火力只覆盖脚下这一排，砸在门上的每一发都不在
   * 僵尸身上，而且这段时间你走不了别的排、吃不到别的排的钱。
   * 打爆了换一把好枪，打不爆什么都没有——Boss 一到，门就没了。
   */
  armory: {
    /** 门吊在方阵前方多远。够近，看得清门上那把枪；够远，永远走不到。 */
    ahead: 34,
  },
  spikes: {
    /** 方阵推进到离墙这么近就开始被刺穿。 */
